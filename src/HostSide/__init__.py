...
class Joystick(mp_module.MPModule):
    ...
    #
    # The beginning of the function writed for RaspiJoy
    # @author: sevvalmehder
    # This function must be called in the line 154
    #
    def select_control(self, pattern, joy, oldJoydef):
    	'''This is a function for RaspiJoy. If the matched joystick is RaspiJoy, 
    	user can use RaspiJoy as another josytick'''
    	...
    	# If the matched joystick is RaspiJoy
    	if pattern == "RaspiJoy Joystick":
    		...
    		# If user select RaspiJoy return the function
			if choice == 0:
				return oldJoydef

			selectedJS = availableJSs[choice]

    		# Search the selected joystick
	 		for newJoydef in self.joydefs:
				for pattern in newJoydef['match']:
					if fnmatch.fnmatch(selectedJS.lower(),
                                       pattern.lower()):
						return newJoydef
    ...
    #
    # The finishing of the function write for RaspiJoy
    #
            ...
            for joydef in self.joydefs:
                ...
                for pattern in joydef['match']:
                    if fnmatch.fnmatch(joy.get_name().lower(),
                    	...
                    	joydef = self.select_control(pattern, joy, joydef)
